...
uint8_t BME280::begin()
{
	...
	writeRegister(BME280_CTRL_MEAS_REG, 0x00); //config oversampling non utilisé et mode sleep pour configurer, doc p27
	
	//Set the config word
	dataToWrite = (settings.tStandby << 0x5) & 0xE0;
	dataToWrite |= (settings.filter << 0x02) & 0x1C;
	writeRegister(BME280_CONFIG_REG, dataToWrite); //configure t_standby en norml mode, coef filtre et spi/i2c 
	...
	writeRegister(BME280_CTRL_HUMIDITY_REG, dataToWrite); //configure l'oversampling pour humudité
	
	//set ctrl_meas
	//First, set temp oversampling
	dataToWrite = (settings.tempOverSample << 0x5) & 0xE0;  //oversampling temperature
	//Next, pressure oversampling
	dataToWrite |= (settings.pressOverSample << 0x02) & 0x1C;  //oversampling pression
	//Last, set mode
